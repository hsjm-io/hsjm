import { MaybeRef, createUnrefFn, debouncedWatch } from '@vueuse/core'
import { ComputedRef, Ref, isRef, ref } from 'vue-demi'
...
export function useGeocoding(searchText?: MaybeRef<string | undefined>, options = {} as UseGeocodingOptions) {
  ...
  const geocode = createUnrefFn((searchText?: string) => {
    ...
    return searchText ? axios.get<UseGeocodingReturn>(url, { params: options }).then(({ data }) => data) : undefined
  })
