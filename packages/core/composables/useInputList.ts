...
export interface UseInputListReturnType<T = any, V = any> {
  /** List of items. */
  items: ComputedRef<ListItem<T, V>[]>
  /** List of selected items. */
  itemsSelected: ComputedRef<ListItem<T, V>[]>
  /** If the item is selected. */
 ...
 * @param {Ref<any>} modelValue Model value.
 * @param {UseInputListOptions} options Composable options.
 * @returns {UseInputListReturnType} Composable.
 * @see `InputList`
 * @example
 * ```ts
 ...
 * ```
 */
export const useInputList = <T, V>(modelValue: Ref<any>, options: UseInputListOptions<T, V>): UseInputListReturnType<V, T> => {
  // --- Is item selected?
  // TODO: Add deep comparaison features.
